...
class DijkstraSearch:
    class Node:
        ...
        def __init__(self, x, y, cost=None, parent=None, edge_ids=None, orientation = 1000):
            self.x = x
            self.y = y
            self.cost = cost
            self.parent = parent
    ...
    def __init__(self, show_animation,mainclass):
        self.show_animation = show_animation
        self.coll_checker = mainclass
    ...
    def search(self, sx, sy, gx, gy, node_x, node_y, edge_ids_list):
        ...
        start_node = self.Node(sx, sy, 0.0, -1, orientation = 0)
        ...
        while True:
            ...
            for i in range(len(edge_ids_list[current_id])):
                n_id = edge_ids_list[current_id][i][0]
                dx = node_x[n_id] - current_node.x
                dy = node_y[n_id] - current_node.y
                d = math.hypot(dx, dy)
                node = self.Node(node_x[n_id], node_y[n_id],
                                 current_node.cost + d, current_id,orientation = edge_ids_list[current_id][i][1])
                #print(34)
                if not self.verify_robot_constraint(current_node,node):
                    continue
                #print(45)
    ...
    def verify_robot_constraint(self,current,node):
        ...
        init = c_orient
        no = 0    
        while(np.abs(init-n_orient)//5 != 0 and np.abs(init-n_orient_c)//5 != 0):
            init = init + 5
            #print(init)
            if init > 360:
                init = init - 360
            if self.coll_checker.is_collision(px,py,init):
                no = 1
                print('Breaking')
                break
        if no == 0:
